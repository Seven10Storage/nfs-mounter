...
public class NfsMounterFactorySettings
{
	...
	/**
	 * If true, NFS requests are retried indefinitely Otherwise, the NFS client
	 * fails an NFS request after retrans retransmissions have been sent,
	 * causing the NFS client to return an error to the calling application
	 * Note: A so-called "soft" timeout can cause silent data corruption in
	...
	/**
	 * If set, allows NFS requests to be interrupted if the server goes down or
	 * cannot be reached.
	 */
	public boolean isRequestInterruptable = true;
	
	/**
	 * This setting speeds up NFS communication for reads by setting a larger
	 * data block size (num, in bytes), to be transferred at one time. Be
	 * careful when changing these values; some older Linux kernels and network
	 * cards do not work well with larger block sizes.
	 */
	public int readDataBlockSize = 8192;
	
	/**
	 * This setting speeds up NFS communication for writes by setting a larger
	 * data block size (num, in bytes), to be transferred at one time. Be
	 * careful when changing these values; some older Linux kernels and network
	 * cards do not work well with larger block sizes.
	 */
	...
	/**
	 * Debug information for the factory settings object
	 */
	@Override
	public String toString()
	{
		String[] params = { String.format("isFsReadOnly=%s,", Boolean.toString(isFsReadOnly)),
				String.format("isRequestsRetriedIndefinitely=%s", Boolean.toString((isRequestsRetriedIndefinitely))),
				String.format("isRequestInterruptable=%s", Boolean.toString(isRequestInterruptable)),
				String.format("readDataBlockSize=%d", readDataBlockSize),
				String.format("writeDataBlockSize=%d", writeDataBlockSize),
				String.format("linuxBaseMntDir='%s'", linuxBaseMntDir),
				String.format("linuxAutoFsTemplatePath='%s'", linuxAutoFsTemplatePath) };
